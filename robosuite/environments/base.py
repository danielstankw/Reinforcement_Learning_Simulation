...
class MujocoEnv(metaclass=EnvMeta):
    ...
    def __init__(
        ...
        self.r_reach_value = r_reach_value
    ...
    def step(self, action):
        ...
        while not done:
            ...
            policy_step = True  # kept to allow option for different functionality (not used)
            ...
            for i in range(int(self.control_timestep / self.model_timestep)):
                # run loop 25 times: 0.05/0.002=25
                self.sim.forward()
                self._pre_action(action, policy_step)  # min-jerk, PD and impedance update in 500HZ
                self.sim.step()  # every step is 0.002 sec
                policy_step = False

            # Note: this is done all at once to avoid floating point inaccuracies
            self.cur_time += self.control_timestep  # current simulation time != real world time
        ...
        info.update({"time": self.cur_time})
        info.update({"error": self.error})
        # simulation time != real time (x axis of plots)
        # df = pd.DataFrame({'t': self.robots[0].controller.time_vec, 'x': self.robots[0].controller.ee_pos_x_vec,
        #                    'y': self.robots[0].controller.ee_pos_y_vec, 'z': self.robots[0].controller.ee_pos_z_vec, 'rx': self.robots[0].controller.ee_ori_x_vec,
        #                    'ry': self.robots[0].controller.ee_ori_y_vec, 'rz': self.robots[0].controller.ee_ori_z_vec, 'vx': self.robots[0].controller.ee_vel_x_vec,
        ...
        # # filename='ep1.csv'
        # filepath = os.path.join('/home/user/Desktop/Daniel_simulation/robosuite/data_collection', filename)
        # df.to_csv(filepath)
        ...
        info.update({"horizon": self.current_horizon})  # horizon
    ...
    def _post_action(self, action):
        ...
        end_horizon = self.current_horizon >= self.horizon
        ...
        self.done = end_horizon or self.end_success or end_termination
