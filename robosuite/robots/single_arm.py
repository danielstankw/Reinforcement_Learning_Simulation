...
class SingleArm(Manipulator):
    ...
    def __init__(
            ...
            # daniel: [-0.22, -1.2, 1.5745819, -1.91207876, -1.57039959, 1.3786325]
        ...
        self.desired_ori_vel = None
        self.desired_vel = None
        self.desired_ori = None
        self.desired_pos = None
        self.via_point_switch = 0
        self.enter = 1
        self.t_bias = 0
        self.t_finial = 0.0
        self.via_point = OrderedDict()
        self.set_initial_point = None
    ...
    def reset(self, deterministic=False):
        ...
        self.enter = 1
        self.t_bias = 0
        self.controller.desired_vec_fin = []
    ...
    def control(self, action, policy_step=False):
        ...
        # if policy_step:
        self.controller.switch = self.via_point_switch
        self.calc_next_desired_point()
    ...
    def calc_next_desired_point(self):
        ...
        if self.sim.data.time == 0:
            self.controller.built_min_jerk_traj(self.t_finial, self.t_bias, self.via_point)

        if self.via_point_switch == 1 and self.enter == 1:
            self.t_bias = self.sim.data.time
            self.enter = 0
            self.controller.built_min_jerk_traj(self.t_finial, self.t_bias, self.via_point)
